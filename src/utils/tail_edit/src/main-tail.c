...
"    max of (thres-sub, del, ins) <= maximum Levenshtein distance\n"
"    if the thresholds (thres-sub, del, ins) are equal to 0, the clustering is conducted by only total Levenshtein distance\n"
...
)
{
   ...
   while (1) {
      ...
      c = getopt_long(argc, argv, "1:2:3:4:x:y:z:d:l:hi:o:qjt:r:v",
      ...
      case 'd':
         if (dist < 0) {
            ...
            if (dist < threshold_sub + threshold_del + threshold_ins){
               fprintf(stderr, " !!!! maximum distance must be larger than sum of thres-sub, del, ins)");
               say_usage();
               return EXIT_FAILURE;
            }
         }
