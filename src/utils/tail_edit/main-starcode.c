...
"       -e --thres-base: threshold of number of base edit distance\n"
"  max of (thres-sub, del, ins) <= maximum Levenshtein distance (--dist)\n";
...
)
{
   ...
   while (1) {
      ...
      static struct option long_options[] = {
         ...
         {"jiyeon-clustering", no_argument,       &jp_flag, 'j'},
         ...
         {"thres-base",        required_argument,        0, 'e'},

         {0, 0, 0, 0}
      };

      c = getopt_long(argc, argv, "1:2:3:4:x:y:z:e:d:hi:o:qcsjt:r:v",
      ...
      case 'e':
         threshold_base = atoi(optarg);
         int max_thres = ((threshold_sub > threshold_del) && (threshold_sub > threshold_ins)) ? threshold_sub : ((threshold_del > threshold_sub) && (threshold_del > threshold_ins)) ? threshold_del : threshold_ins;
         if (dist < max_thres + threshold_base){
            fprintf(stderr, " !!!! maximum distance must be larger than sum(threshold_base, max of thres-sub, del, ins)");
            say_usage();
            return EXIT_FAILURE;
         }
         break;
   ...
   else if (jp_flag) cluster_alg = JIYEON_CLUSTER;
